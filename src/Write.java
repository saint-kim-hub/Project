...
public class Write {
	public static void writeFile(Packet packet) throws Exception {
		Path path = Paths.get("/opt/tomcat/2019", Format.date() + ".txt"); // 패킷을 저장할 경로 지정. /opt/tomcat/년도/날짜.txt
//		Path path = Paths.get("d://2019", Format.date() + ".txt");
		...
		Files.write(path, contents, charset, createOpt, appendOpt); // 파일 저장
	}
}